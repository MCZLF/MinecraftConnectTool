...
namespace MinecraftConnectTool
{
    internal class supportcheck
    {
        public static void Check()
        {
            try
            {
                var list = new WebClient().DownloadString("https://api.mct.mczlf.loft.games/006/SupportVer").Split('\n');
        ...
        static async void NotSupport()
        {
            try
            {
                var cloud = (await new WebClient().DownloadStringTaskAsync("https://api.mct.mczlf.loft.games/006/version006")).Trim();
